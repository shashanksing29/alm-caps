...
import { getALMConfig, contentLocale } from "../almLib/utils/global";
...
export const GetManifestResourcePath = (key: string): string | undefined => {
  ...
  return (
    window.location.origin + getALMConfig().frontendResourcesPath + "/" + val
  );
};
...
const computeUserLocale = (): string => {
  ...
  if (almConfig) {
    ...
    locale = contentLocale().includes(languagecode) ? languagecode : ENGLISH_LOCALE;
    if (languagecode === "es-LA")
      locale = "es-ES";
    if (languagecode.startsWith("ar-")) {
      locale = "ar-SA";
    }
    if (languagecode.startsWith("sl-")) {
      locale = "sl-SL";
    }
  }
