...
> {
  ...
  public async getAccessTokenAPI(): Promise<any> {
    const tokenData = await fetch("/bin/startatibm/gettoken");
    return tokenData.json();
  }

  public async privacyAcceptanceAPI(userEmail: string, token: string): Promise<any> {
    if (userEmail && token) {
      const params = new URLSearchParams({ email: userEmail, token });
      const privacyData = await fetch(`/bin/startatibm/privacyacceptance?${params}`);
      return privacyData;
    }
  }
}
