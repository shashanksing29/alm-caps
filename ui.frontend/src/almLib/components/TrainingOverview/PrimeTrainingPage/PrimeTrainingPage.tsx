...
const PrimeTrainingPage = (props: any) => {
  ...
  const rtlLanguageCodes = ["ar-SA", "he-IL"];
  ...
  if (isLoading || !training || isInstancePageLoading) {
    return <ALMLoader id={"loader-icon"} classes={styles.loader} />;
  }
            ...
            <div className={styles.left}>
              {trainingOverviewAttributes.showDescription === "true" && (
                <div
                  ...
                  className={`${styles.description} body-long-01 text-sans ${(rtlLanguageCodes.includes(props.locale)) ? "" : "ql-editor"}`}
                ></div>
              )}
                ...
                {!isFlexible ? (
                  <>
                   {formatMessage(
                      ...
                      { 0: convertSecondsToTimeText(training.duration, locale) }
                    )}
                  </>
                ) : (
                  <>
                   {formatMessage(
                      { id: "alm.overview.total.duration" },
                      { 0: convertSecondsToTimeText(timeDuration(), locale) }
                    )}
                  </>
                )}
              </span>
