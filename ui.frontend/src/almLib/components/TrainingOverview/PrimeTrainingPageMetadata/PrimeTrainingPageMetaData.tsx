...
import { Pin, DocumentBlank } from "@carbon/icons-react";
...
const PrimeTrainingPageMetaData: React.FC<{
  ...
  const enrollmentClickHandler = async (launchPlayer = false) => {
    try {
      const account = await getALMAccount();
  ...
  const coreContentCompleted = useMemo(() => {
    ...
    if (loType === COURSE) {
      const preWorkResources = filterLoReourcesBasedOnResourceType(trainingInstance, PREWORK);
      const preWorkResourceIds: string[] = [];
      preWorkResources.forEach((resource) => {
        preWorkResourceIds.push(resource?.id);
      });
      const testOutResources = filterLoReourcesBasedOnResourceType(trainingInstance, TESTOUT);
      const testOutResourceIds: string[] = [];
      testOutResources.forEach((resource) => {
      testOutResourceIds.push(resource?.id);
      });
      const totalCount = coreContentModules?.length;
      ...
      enrollment?.loResourceGrades?.forEach(
        (item: PrimeLearningObjectResourceGrade) => {
          if (item.hasPassed) {
            let itemResourceId = item?.id.replace(/_\d+$/, '');
            if(!preWorkResourceIds.includes(itemResourceId) && !testOutResourceIds.includes(itemResourceId))
              completionCount += 1;
          }
        }
      );
  ...
  const downloadFile = (location: string, name: string) => {
    axios({
      url: location,
      method: 'GET',
      responseType: 'blob',
  ...
  useEffect(() => {
    // if(isPrimeUserLoggedIn && !checkIsEnrolled(training.enrollment)) {
    //   handleEnrollment();
    // }
  ...
  return (
    <section className={styles.container}>
      ...
      <section className={styles.borderContainer}>
          ...
          <div className={styles.commonContainer}>
            ...
            <div className={styles.innerContainer}>
              ...
              {trainingInstance?.validity
                    ? GetTranslation("alm.certification.recurring", true)
                    : GetTranslation("alm.certification.nonRecurring", true)}
              </div>
            </div>
          </div>
        )}
        ...
        {showJobAids && (
          <div className={styles.commonContainer}>
            <span aria-hidden="true" className={styles.carbonIcon}>
              <Pin size={24} />
            </span>
            <div className={`${styles.innerContainer} ${styles.jobAidsAndResources}`}>
              <label className={styles.label}>
