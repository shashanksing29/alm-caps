...
import { Time, CheckmarkFilled, LocationStarFilled, ConditionWaitPoint, InProgress, Upload } from "@carbon/icons-react";
...
const PrimeModuleItem: React.FC<{
  ...
  const attemptDurationText = attemptDuration === 1 ? GetTranslation("alm.text.1min") : GetTranslationReplaced("alm.text.mins", `${attemptDuration}`);
  ...
  const durationText = convertSecondsToTimeText(
    (resource.authorDesiredDuration || resource.desiredDuration || 0), locale);

  const moduleIcon =
    moduleIconMap[resource.contentType as keyof ActionMap] || SCORM_SVG();

  const moduleIconLP =
    moduleIconMap[resource.contentType + "LP" as keyof ActionMap] || SCORM_SVG();
  ...
  const itemClickHandler = (event: any, reattemptButtonClicked = false) => {
    ...
    if (multipleAttempt && attemptsLeft <= 0 && !infiniteAttempts && stopAttemptOnSuccessfulComplete) {
      if (loResourceGrade?.hasPassed === true) {
        return;
      }
    }
    ...
    // Case when no attempt is left and course is passed
    // if (multipleAttempt && attemptsLeft <= 0 && !infiniteAttempts) {
    //     return;
    // }

    // Case when attempt is left and course is passed
    // if (multipleAttempt && attemptsLeft > 0 && !infiniteAttempts) {
    //     return;
    // }
    ...
    if (
      ...
      const isResetReattemptRequired = reattemptButtonClicked && (loResource.learnerAttemptInfo || infiniteAttempts)
        ? true
        : isResetRequired;
      launchPlayerHandler({
        id: training.id,
  ...
  const getFileUploadSection = (hasSession: Boolean, state: String, moduleFileSubmission: Boolean = false) => {
    const allowFileUpdate = true;
    let fileNotAccessibleHtml = <></>;
    if (loResource.isExpiredSubmission) {
      fileNotAccessibleHtml = (
        <div className={moduleFileSubmission ? styles.fileSubmissionWrapper : ''}>
    ...
    switch (state) {
      ...
      case PENDING_APPROVAL:
        return (
          <div className={moduleFileSubmission ? styles.fileSubmissionWrapper : ''}>
            <span className={`${styles.fileSubmissionContainer} m-0`}>
              <ConditionWaitPoint
                size={16}
                aria-label={GetTranslation("alm.overview.submissionAwaitingApproval.label", true)}
              />
      ...
      default:
        return (
          <div className={styles.fileSubmissionWrapper}>
  ...
  const getUploadFileSection = (hasSession: Boolean, state?: String) => {
    return (
      showUpload(hasSession) && (
        <span>
          <div tabIndex={0} onClick={startFileUpload} className={styles.uploadButton} aria-label={GetTranslation("alm.overview.module.uploadSubmission", true)}>
            {state === CHANGE
              ? `(${formatMessage({
                id: "alm.module.change",
                defaultMessage: "Change",
              })})`
              : <span className={styles.durationAndStatus}>
                <Upload
                  style={{ fill: 'var(--prime-link-primary-color)' }}
                  size={16}
                  aria-label={GetTranslation("alm.overview.module.uploadSubmission", true)}
                />
                <label className={`${styles.cursor}` + " link-primary"}>
                  {formatMessage({
                    id: "alm.overview.module.uploadSubmission",
                    defaultMessage: "Upload submission",
                  })}
                </label>
              </span>
            }
  ...
  const clickSubmissionUrl = (url: string, event: any) => {
    ...
    return (
      <>
        <div
          ...
          onClick={(event: any) => clickSubmissionUrl(submissionUrl, event)}
        >
          <label className={`${styles.cursor}` + " link-primary"}>
  ...
  if (
    ...
    if (
      ...
      if (loResourceGrade?.duration === 0) { 
        ...
        attemptsLeft =
          (maxAttemptCount || 0) -
          (learnerAttemptInfo?.attemptsFinishedCount || 0); //fix 1526
      }
    } else if ( 
      loResourceGrade?.hasPassed &&
      loResourceGrade?.progressPercent === 100
      ...
      if (loResourceGrade?.duration === 0) { 
        attemptsLeft =
          (maxAttemptCount || 0) -
          (learnerAttemptInfo?.currentAttemptNumber || 0); //fix 1690
      } else {
        attemptsLeft =
          (maxAttemptCount || 0) -
  ...
  const resourceClickHandler = (event: any) => {
    if (
      ...
      almAlert(
        true,
        GetTranslationReplaced("alm.mqa.module.locked.message", showTimeRemainingBetweenAttempts(), true),
        AlertType.error
      );
    }
  };
  ...
  const showTimeRemainingBetweenAttempts = () => {
    const now = new Date().getTime();
    const lastAttemptEndTime =
      new Date(
        loResource.learnerAttemptInfo?.currentAttemptEndTime ||
    ...
    const timeRemainingInMili = attemptStartTime - now;

    const totalSeconds = Math.floor(timeRemainingInMili / 1000); // Convert to total seconds   
    const days = Math.floor(totalSeconds / 86400); // Calculate days
    const hours = Math.floor((totalSeconds % 86400) / 3600); // Remaining hours
        ...
        if (!loResource.learnerAttemptInfo) {
          return (
            <span className={styles.attemptDescriptionContainer}>
              ...
              <span className={styles.initialAttempts}>
                {GetTranslation("alm.mqa.noLimit", true)}
              </span>
              {attemptDuration !== 0 && (
                <span className={styles.attemptsDuration}>
                  {isMobileScreen ? `| ` : ``}
          ...
          return (
            <span className={styles.attemptDescriptionContainer}>
              {attemptsLeft <= 0 ? (
                ...
                <span className={styles.attemptDescriptionContainer}>
                  <div
                    onClick={attemptsLeft > 0 ? reattemptHandler : undefined}
                    className={
                      moduleLockedBetweenAttempt() || attemptsLeft === 0
                        ? styles.reattemptButtonDisabled
                        : styles.reattemptButton
                    }
                    tabIndex={0}
                    aria-disabled={attemptsLeft === 0}
                  >
                    <span className={styles.refresh}>
                      <Refresh />
                    </span>
  ...
  return (
    <>
      {!isPartOfLP && (
        <ContainedListItem
          className={`${styles.containedListItem} ${loResourceMandatory() ? styles.mandatoryModuleContainer : ``
            }`}
          onClick={resourceClickHandler as () => void}
          data-test={loResource.id}
        >
          <div
            className={`${styles.headerContainer} ${isModuleClickable ? styles.cursor : ""
              }`}
            tabIndex={0}
            data-test={loResource.id}
            ...
            <div className={styles.headerWrapper}>
              <div className={styles.resource}>
                <span className={styles.resourceType}>
                  {`${formatLabel}` + ": " + `${resource.contentType}`}
                  {isUploading && (
              ...
              <div className={styles.titleContainer}>
                <span className={styles.title} aria-label={resource.contentType}>
                  {name}{" "}
                  {showPreWorkLabel
                    ? formatMessage({
                ...
                {isLastPlayedModule ? (
                  <div className={styles.lastVisitedWrapper}>
                    <LocationStarFilled
                      size={21}
                      style={{ fill: 'var(--prime-color-black)' }}
                      aria-label={GetTranslation("alm.module.lastVisited", true)}
                    />
              ...
              <div className={styles.flex}>
                <div className={styles.durationAndStatus}>
                  {" "}
                  <Time size={16} aria-label={GetTranslation("alm.catalog.filter.duration.label", true)} />
                  {durationText}
                </div>
                {gradeHasPassed() ? (
                  <div className={styles.durationAndStatus}>
                    {" "}
                    <CheckmarkFilled
                      style={{ fill: 'var(--prime-completed-color)' }}
                      size={16}
                      aria-label={GetTranslation("alm.status.completed", true)}
                    />
                    <div className={styles['complete-date-wrapper']}>
                      <label className={styles['label-black']}>
                        {GetTranslation("alm.status.completed", true)}
                      </label>
                      {loResourceGrade.dateSuccess && (
                        <label className={styles['label-black']}>
                          {GetFormattedDate(loResourceGrade.dateSuccess, locale, 'dd MMM yyyy')}
                        </label>
                      )}
                    </div>
                  </div>
                ) : checkIfTypeDate(loResourceGrade.dateStarted) ? (
                  <div className={styles.durationAndStatus}>
                    <InProgress
                      size={16}
                      aria-label={GetTranslation("alm.overview.label.inProgress", true)}
                    />
                    <label className={styles['label-black']}>
                      {GetTranslation("alm.overview.label.inProgress", true)}
                    </label>
                  </div>
                ) : !canPlay && !isPreworkModule ? (
                  <div className={styles.durationAndStatus}>
                    <LockClosed size="S" aria-hidden="true" />
                  </div>
                ) : (
                  ""
                )}
              ...
              {!isUploading &&
                showFileSubmission() &&
                getFileUploadSection(isClassroomOrVC, submissionState, true)}
      ...
      {isPartOfLP && (
        <>
          <li
            className={`${styles.container} ${loResourceMandatory() ? styles.mandatoryModuleContainer : ``
              }`}
          >
            <div
              className={`${styles.headerContainer} ${isModuleClickable ? styles.cursor : ""
              ...
              role="button"
            >
              <div className={`${styles.lp} ${styles.icon}`} aria-hidden="true">
                {loResourceMandatory() ? (
                  <span className={styles.mandatoryModule}>
                    <Asterisk />
                  </span>
                ...
                {(resource.contentType === "VIDEO" || resource.contentType === "PDF") ? moduleIconLP : moduleIcon}
                {gradeHasPassed() ? (
                  <span className={styles.modulePassed}>
                    <CheckmarkCircle aria-hidden="true" />
                  </span>
              ...
              <div className={`${styles.lp} ${styles.headerWrapper}`}>
                <div className={styles.titleContainer}>
                  <span className={styles.title} aria-label={resource.contentType}>
                    {name}{" "}
                    {showPreWorkLabel
