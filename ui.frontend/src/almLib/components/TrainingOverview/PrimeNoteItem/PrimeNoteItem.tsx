...
const PrimeNoteItem: React.FC<{
  ...
  const formattedNoteMarker= (note: PrimeNote)=> {
    ...
    if (moduleType === ELEARNING) {
      if (
        ...
        if (hours !== 0) {
          noteMarker =
            noteMarker + str_minutes + ':' + str_sec;
            noteMarker= GetTranslationReplaced("alm.text.hours", `${noteMarker}`);
        } else {
          if (str_minutes === '00') {
            noteMarker =
              noteMarker + str_minutes + ':' + str_sec ;
              noteMarker= GetTranslationReplaced("alm.text.seconds", `${noteMarker}`);
          } else {
            noteMarker =
              noteMarker + str_minutes + ':' + str_sec ;
              noteMarker= GetTranslationReplaced("alm.text.minutes", `${noteMarker}`);
          }
        }
        return noteMarker;
      } else if (
  ...
  return (
    <React.Fragment key={`note-${note.id}`}>
      <div className={`${noteContainer}` + " text-sans"}>
