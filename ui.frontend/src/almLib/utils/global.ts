...
export const getPathParams = (pagePath: string, pathParams: string[] = []) => {
  ...
  const pathname =
    currentUrl.pathname.indexOf(pagePath) > -1 ? currentUrl.pathname : pathVal;
...
export function updateURLParams(params: any) {
  ...
  window.parent.postMessage(
    { data: convertUrlToEmber(), origin: window.origin, href: newurl },
    "*"
  );
}
...
export const setUserCookie = (key: string) => {
  return getALMObject().setUserCookie(key);
};

export const getCookieByName = (key: string) => {
  return getALMObject().getCookieByName(key);
};

export const sitePath = () => {
  return getALMObject().sitePath;
};

export const contentLocale = () : string[] => {
  return getALMObject().contentLocale;
};
...
export const checkIfLinkedInLearningCourse = (
  training: PrimeLearningObject
) => {
