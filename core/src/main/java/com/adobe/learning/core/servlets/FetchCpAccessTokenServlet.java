...
public class FetchCpAccessTokenServlet extends SlingAllMethodsServlet {
  ...
  protected void doPost(SlingHttpServletRequest request, SlingHttpServletResponse response) {
    try {
      ...
      boolean storeTokenInCookie = jsonConfigs.get(Constants.Config.NOT_STORE_TOKEN_IN_COOKIE) != null
          ? "false"
              .equals(jsonConfigs.get(Constants.Config.NOT_STORE_TOKEN_IN_COOKIE).getAsString())
          : true;
      boolean useAdminRefreshToken = jsonConfigs.get(Constants.Config.USE_ADMIN_RT_TO_LEARNER_AT) != null
      ...
      if (AUTHOR_MODE.equals(mode)) {
        ...
        if (Constants.Config.SITES_USAGE.equals(usageType)) {
          refreshToken = useAdminRefreshToken
        ...
        Pair<String, Integer> accessTokenResp = tokenService.getAccessToken(almURL, clientId, clientSecret,
            refreshToken);
        ...
        if (Constants.Config.COMMERCE_USAGE.equals(usageType) || useAdminRefreshToken) {
          ...
          String learnerTokenResponse = tokenService.fetchLearnerToken(almURL, clientId, clientSecret, refreshToken,
              email);
          if (!containValidAccessToken(learnerTokenResponse)) {
            ...
            accessTokenResp = new ImmutablePair<String, Integer>(
        ...
        Pair<String, Integer> accessTokenResp = tokenService.getAccessTokenFromCode(almURL, clientId, clientSecret,
            code);
  ...
  private void setAccessTokenCookie(
    ...
    if (refHeader.contains("/content/ibm-internal"))
      tokenCookie.setPath("/content/ibm-internal");
    else if (refHeader.contains("ibm-internal"))
      tokenCookie.setPath("/ibm-internal");
    else if (refHeader.contains("/content/ibm-training"))
      tokenCookie.setPath("/content/ibm-training");
    else if (refHeader.contains("ibm-training"))
      tokenCookie.setPath("/ibm-training");
    else if (refHeader.contains("/content/ibm-skillbuild-highschool"))
      tokenCookie.setPath("/content/ibm-skillbuild-highschool");
    else if (refHeader.contains("ibm-skillsbuildhighschool"))
      tokenCookie.setPath("/ibm-skillsbuildhighschool");
    else if (refHeader.contains("/content/ibm-skillbuild-adult"))
      tokenCookie.setPath("/content/ibm-skillbuild-adult");
    else if (refHeader.contains("ibm-skillsbuildadult"))
      tokenCookie.setPath("/ibm-skillsbuildadult");
    else if (refHeader.contains("/content/ibm-startatibm"))
      tokenCookie.setPath("/content/ibm-startatibm");
    else if (refHeader.contains("ibm-startatibm"))
      tokenCookie.setPath("/ibm-startatibm");
