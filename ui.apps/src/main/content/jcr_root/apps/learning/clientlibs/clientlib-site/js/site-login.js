...
(async function (window, document, Granite, $) {
  ...
  async function getALMUser() {
    if (!isPrimeUserLoggedIn()) {
      ...
      document.cookie = "user=; path=" + window.ALM.sitePath + "; Expires=Thu, 01 Jan 1970 00:00:01 GMT;";
      return;
    }
    ...
    const userUrl = `${primeApiURL}/user`;
    const headers = {
  ...
  function setCookie(userStr) {
    document.cookie = "user=" + userStr + "; path=" + window.ALM.sitePath;
  }
  ...
  window.ALM.setUserCookie = setCookie;
  window.ALM.getCookieByName = getCookieByName;
