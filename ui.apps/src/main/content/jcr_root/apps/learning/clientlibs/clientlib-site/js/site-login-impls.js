(function (window) {
  ...
  function getPathValue(pathVal) {
    let newPath = "";
    if (pathVal.includes("/ibm-internal")) {
      newPath = pathVal.replace(
        "/ibm-internal",
        "/content/ibm-internal/us/en/overview.html"
      );
    }
    if (pathVal.includes("/ibm-training")) {
      newPath = pathVal.replace(
        "/ibm-training",
        "/content/ibm-training/us/en/overview.html"
      );
    }
    if (pathVal.includes("/ibm-skillsbuildhighschool")) {
      newPath = pathVal.replace(
        "/ibm-skillsbuildhighschool",
        "/content/ibm-skillbuild-highschool/us/en/overview.html"
      );
    }
    if (pathVal.includes("/ibm-skillsbuildadult")) {
      newPath = pathVal.replace(
        "/ibm-skillsbuildadult",
        "/content/ibm-skillbuild-adult/us/en/overview.html"
      );
    }
    if (pathVal.includes("/ibm-startatibm")) {
      newPath = pathVal.replace(
        "/ibm-startatibm",
        "/content/ibm-startatibm/us/en/overview.html"
      );
    }
    return newPath;
  }
  class AlmLogin {
    ...
    getCpOauthUrl() {
      const { almBaseURL, clientId, accountId, ipId, accessKey } =
        window.ALM.ALMConfig;
      if (ipId && accessKey) {
        return `${almBaseURL}/oauth/o/authorize?client_id=${clientId}&redirect_uri=${window.location.href}&state=${CP_OAUTH_STATE}&scope=learner:read,learner:write&response_type=CODE&client_identifier=aemsite&logoutAfterAuthorize=true&loginUrl=%2Faccountiplogin%3FipId%3D${ipId}%26accesskey%3D${accessKey}`;
      } else {
    ...
    async cleanUpUserData() {
      ...
      ["user", "CART_ID", "PRIME_CATALOG_FILTER", "COMMERCE_FILTERS"].forEach(
        (param) => window.ALM.storage.removeItem(param)
      );
    }
    ...
    showPopup(errorMsg, variant, header) {
      ...
      dialogModal = new Coral.Dialog().set({
        ...
        footer: {
          innerHTML:
            '<button is="coral-button" variant="default" coral-close>OK</button>',
        },
      });
    ...
    async fetchAccessToken(data) {
      ...
      try {
        ...
        if (response && response.status == 200) {
          ...
          ["PRIME_BASE", "code", "state"].forEach((param) =>
    ...
    async checkALMLoginImpl() {
      const REDIRECT_URL = Granite.HTTP.externalize(
        CP_ADMIN_RT_LEARNER_RT_TOKEN_URL
      );
      const requestData = {
        _charset_: "UTF-8",
      };
      ...
      if (pathName != null || pathName != undefined) {
        const account = pathName.split("/");
        if (accountsALM.includes(account[1])) {
          pathName = getPathValue(pathName);
        }
      }
      ...
      try {
        ...
        if (
          response &&
          response.status == 200 &&
          jsonRes.isALMLoginImplementation === true
        ) {
    ...
    async setAccessToken() {
      try {
        const resp = await this.makeAjaxRequest(
          PROXY_ALM_LOGIN_URL,
          "POST",
          {}
        );
    ...
    async handlePrimeLogIn() {
      ...
      if (!isLoggedIn) {
        ...
        if (window.ALM.isAuthor()) {
          ...
          window.ALM.ALMConfig["useAdminRefreshToken"] === "true" &&
          code === null
        ) {
  ...
  class AlmPrimeLogin extends AlmLogin {
    ...
    async handlePageLoad() {
      ...
      if (getCookieByName("alm_cp_token")) {
        isLoggedIn = true;
        window.ALM.almAccessToken = getCookieByName("alm_cp_token");
      } else {
        console.error("Error in checking logged in");
        isLoggedIn = false;
        window.ALM.almAccessToken = "";
      ...
      if (
        window.ALM.isSignOutPage() ||
        window.ALM.isCommerceSignInPage() ||
        window.ALM.isEmailRedirectPage()
      ) {
        return;
      }
  ...
  class AlmEsLogin extends AlmLogin {
    ...
    async handlePageLoad() {
      ...
      if (window.ALM.isAuthor()) {
        ...
        if (ES_REGISTER_STATE == state && code) {
          ...
          window.ALM.isCommunityPage() ||
          window.ALM.isBoardDetailsPage() ||
          window.ALM.isBoardsPage() ||
          window.ALM.isProfilePage() ||
          (CP_OAUTH_STATE == state && code)
        ) {
    ...
    handleRegister() {
      ...
      document.location.href = `${almBaseURL}/oauth/o/authorize?client_id=${clientId}&redirect_uri=${
        window.location.href
      }&state=${ES_REGISTER_STATE}&scope=learner:read,learner:write&response_type=CODE&client_identifier=aemsite&logoutAfterAuthorize=true&loginUrl=${encodeURIComponent(
        almRegisterUrl
      )}`;
    }
  ...
  class AlmCommerceLogin extends AlmLogin {
    ...
    async handlePageLoad() {
      ...
      try {
        const resp = await super.makeAjaxRequest(
          PROXY_ALM_COMMERCE_LOGIN_URL,
          "POST",
          {}
        );
        if (resp && resp.response && resp.status === 200) {
  ...
  window.ALM.loginImpls = window.ALM.loginImpls || {
    AlmPrimeLogin: new AlmPrimeLogin(),
    AlmEsLogin: new AlmEsLogin(),
    AlmCommerceLogin: new AlmCommerceLogin(),
  };
})(window);
